...
class Oracle2Postgres(object):
    ...
    config['password'] = 'oracle'#getpass.getpass('- Password for source database: ')
...
def get_target_config():
    ...
    config = {}
    config['username'] = 'sudh'
    config['host'] = 'localhost'
    config['port'] = 5432
    config['database'] = "hr"
    config['password'] = 'sudh@123'#getpass.getpass('- Password for target database: ')
...
def get_migration_config():
    ...
    if config['trialrun']:
        config['batchsize'] = min(config['batchsize'],1100000)
